...
module.exports = (teardown) => {
  ...
  listenForShutdown(server, player, teardown)
...
function registerTools(server, player) {
  ...
  server.tool('play', {
    ...
    handler: ({ index, track }) => {
      ...
      const i = resolveIndex(player, index, track)
      player.play(i)
...
function resolveIndex(player, index, track) {
  if (index !== undefined) {
    const i = requireIndex(index)
...
function listenForShutdown(server, player, teardownHook) {
  let closed = false
