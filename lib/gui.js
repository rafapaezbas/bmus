...
class Preview {
  ...
  refresh(current) {
    ...
    setItemsKeepingPosition(this.list, display)
  }
}
...
class TextInput {
  ...
  submit(msg) {
    if (this.input.open) {
      ...
      return fn ? fn() : null // all register commands must return a Message
    }
  }
  ...
  update(msg) {
    const [, cmd] = this.input.update(msg)
    return cmd
  }
...
function setItemsKeepingPosition(l, display) {
  const selected = l.selected
  const offset = l.offset
  l.setItems(display)
  l.selected = Math.max(0, Math.min(selected, display.length - 1))
  l.offset = Math.max(0, Math.min(offset, l.selected))
  if (l.height > 0 && l.selected >= l.offset + l.height) l.offset = l.selected - l.height + 1
}
